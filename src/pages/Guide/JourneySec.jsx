...
const JourneySec = () => {
  ...
  const steps = [
  { id: 1, title: tr("guide_step_1_title"), desc: tr("guide_step_1_desc"), image: feature1 },
  { id: 2, title: tr("guide_step_2_title"), desc: tr("guide_step_2_desc"), image: feature2 },
  { id: 3, title: tr("guide_step_3_title"), desc: tr("guide_step_3_desc"), image: feature3 },
  { id: 4, title: tr("guide_step_4_title"), desc: tr("guide_step_4_desc"), image: feature4 },
  { id: 5, title: tr("guide_step_5_title"), desc: tr("guide_step_5_desc"), image: feature5 },
  { id: 6, title: tr("guide_step_6_title"), desc: tr("guide_step_6_desc"), image: feature6 },
];
  ...
  return (
    <section id='how_it_works' className="font-sans bg-[#F0F1F2] py-12 md:py-24">
      <div className="max-w-7xl px-8 mx-auto">
        
        <div className='mb-0 md:mb-8 mx-auto px-6  text-center flex flex-col items-center-safe '>
          <h3 className="font-Buenard font-bold xs:text-Paragraph4 lg:text-Paragraph2 text-textGreen">
            {tr("guide_journey_label")}
          </h3>
          ...
          <div className="w-full lg:w-1/3 order-2 lg:order-1">
            <div className={`p-2 lg:pt-0 rounded-2xl lg:rounded-none flex  w-[98%] ml-[1%]  lg:ml-0 lg:w-full lg:bg-[#F0F1F2] bg-white lg:flex-col justify-around flex-row lg:space-y-5`}>
              {steps.map((step) => (
                <div 
                  key={step.id} 
                  className={`flex cursor-pointer justify-around md:justify-start transition-all duration-300 w-full
                    ${activeStep === step.id ? "lg:bg-white  lg:p-5 rounded-2xl shadow-sm" : "lg:bg-gray-50  lg:p-5 rounded-2xl hover:lg:bg-white/50"}
                  `}
                  onClick={() => handleStepClick(step.id)}
                >
                  <div 
                    className="shrink-0 text-left w-8 h-8 flex items-center justify-center rounded-full text-lg font-bold ltr:mr-4 rtl:ml-4 transition-all duration-300"
                    style={{ 
        ...
        <div className="lg:hidden w-[95%] ml-[2.5%] bg-white rounded-2xl mt-4 p-4" >
          ...
          <p className='font-Urbanist' style={{color : DARK_GRAY}}>{steps[activeStep-1]?.desc}</p>
        </div>
